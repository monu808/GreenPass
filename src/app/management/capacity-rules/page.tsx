              ...
              <div className="md:hidden divide-y divide-gray-100">
                {loading ? (
                  ...
                  destinations.map((dest) => {
                    ...
                    return (
                      <div key={dest.id} className="p-4 space-y-4">
                        <div className="flex justify-between items-start">
                          <div>
                            <div className="font-bold text-gray-900">{dest.name}</div>
                            <div className="text-xs text-gray-500 capitalize">{dest.ecologicalSensitivity} sensitivity</div>
                          </div>
                          <span className={`text-sm font-black px-2 py-1 rounded-lg ${
                            (result?.factors.combinedMultiplier || 1) < 0.7 ? 'bg-red-50 text-red-600' :
                            (result?.factors.combinedMultiplier || 1) < 0.9 ? 'bg-orange-50 text-orange-600' : 'bg-green-50 text-green-600'
                          }`}>
                            {Math.round((result?.factors.combinedMultiplier || 1) * 100)}%
                          </span>
                        </div>
                        ...
                        <div className="flex items-center justify-between text-sm">
                          <span className="text-gray-500">Base / Adjusted:</span>
                          <div className="font-bold text-gray-900">
                            <span className="text-gray-400 font-normal">{dest.maxCapacity}</span>
                            <ArrowRight className="h-3 w-3 inline mx-2 text-gray-300" />
                            <span>{result?.adjustedCapacity || 0}</span>
                          </div>
                        </div>

                        <div className="space-y-1.5">
                          <div className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">Active Factors</div>
                          <div className="flex flex-wrap gap-1.5">
                            {result?.activeFactors.map((factor, i) => (
                              <span key={i} className="inline-flex items-center px-2 py-1 rounded-md text-[10px] font-bold bg-gray-100 text-gray-700 border border-gray-200">
                                {factor}
                              </span>
                            ))}
                            {result?.activeFactors.length === 0 && (
                              <span className="text-xs text-gray-400 italic">No active factors</span>
                            )}
                          </div>
                        </div>
                        ...
                        <div className="pt-2">
                          {hasOverride ? (
                            <button
                              onClick={() => handleClearOverride(dest.id)}
                              disabled={isSavingOverride}
                              className="w-full min-h-[44px] flex items-center justify-center gap-2 bg-red-50 text-red-600 font-bold rounded-xl border border-red-100 active:scale-[0.98] transition-all"
                            >
                              {isSavingOverride ? (
                                <RefreshCw className="h-4 w-4 animate-spin" />
                              ) : (
                            ...
                            <button
                              ...
                              disabled={isSavingOverride}
                              className="w-full min-h-[44px] flex items-center justify-center bg-green-50 text-green-600 font-bold rounded-xl border border-green-100 active:scale-[0.98] transition-all"
                            >
                              Add Override
                            </button>
                          )}
