...
export const downloadCompliancePDF = (
  ...
  if (report.ecologicalDamageIndicators) {
    ...
    autoTable(doc, {
      ...
      body: [
        ["Soil Compaction", (report.ecologicalDamageIndicators.soilCompaction ?? 0).toFixed(2), (report.ecologicalDamageIndicators.soilCompaction ?? 0) > 0.7 ? "HIGH" : "NORMAL"],
        ["Vegetation Disturbance", (report.ecologicalDamageIndicators.vegetationDisturbance ?? 0).toFixed(2), (report.ecologicalDamageIndicators.vegetationDisturbance ?? 0) > 5 ? "ACTION REQ" : "LOW"],
        ["Wildlife Disturbance", (report.ecologicalDamageIndicators.wildlifeDisturbance ?? 0).toFixed(2), (report.ecologicalDamageIndicators.wildlifeDisturbance ?? 0) > 5 ? "ACTION REQ" : "LOW"],
        ["Water Source Impact", (report.ecologicalDamageIndicators.waterSourceImpact ?? 0).toFixed(2), (report.ecologicalDamageIndicators.waterSourceImpact ?? 0) > 10 ? "MONITOR" : "LOW"],
      ],
      theme: "striped",
