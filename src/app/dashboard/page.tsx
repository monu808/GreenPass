        ...
        <div className="max-w-7xl mx-auto space-y-10 pb-20 px-6">
          ...
          >
            <div className="flex items-center justify-between px-1">
              <h2 id="top-destinations-title" className="text-3xl font-black text-gray-900 dark:text-white tracking-tighter">Top Destinations</h2>
              <button
                type="button"
                ...
                {featuredDestinations.map((dest) => {
                  ...
                  return (
                    <div key={dest.id} className="space-y-4">
                      <div className="group bg-white dark:bg-slate-800 rounded-[2.5rem] border border-gray-100 dark:border-slate-700 overflow-hidden shadow-sm hover:shadow-2xl transition-all duration-500">
                        ...
                        <div className="p-6 bg-gray-50/50 dark:bg-slate-700/50 border-t border-gray-100 dark:border-slate-600 flex items-center justify-center text-[11px] text-gray-500 dark:text-gray-400 font-bold uppercase tracking-widest min-h-[4rem]" role="status" aria-live="polite">
                          {weather ? (
                      ...
                      {(() => {
                        ...
                        return (
                          <div 
                            className="bg-emerald-50/50 dark:bg-emerald-900/20 rounded-[2rem] p-6 border border-emerald-100 dark:border-emerald-800 animate-in slide-in-from-bottom-4 duration-500"
                            role="region"
                          ...
                          >
                            ...
                            <p className="text-[11px] text-emerald-800/70 dark:text-emerald-300/70 font-bold mb-4">Consider these lower-impact alternatives to reduce ecological pressure on {dest.name}:</p>
                            <div className="grid grid-cols-1 gap-3">
                              {alternatives.map(alt => (
                                <button
                                  key={alt.id}
                                  onClick={() => handleNavigation(`/tourist/destinations?search=${alt.name}`)}
                                  className="bg-white dark:bg-slate-800 p-4 rounded-2xl border border-emerald-100 dark:border-emerald-800 flex items-center justify-between group hover:border-emerald-300 dark:hover:border-emerald-600 hover:shadow-md transition-all text-left focus:outline-none focus:ring-4 focus:ring-emerald-500/10"
                                  aria-label={`View alternative destination: ${alt.name}`}
                                >
                                  <div className="space-y-1">
                                    <h5 className="text-sm font-black text-gray-900 dark:text-gray-100 group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors">{alt.name}</h5>
          ...
          >
            {[
              ...
              <div 
                key={i} 
                className="bg-white dark:bg-slate-800 rounded-[2.5rem] p-8 shadow-sm border border-gray-100 dark:border-slate-700 flex items-center justify-between"
                role="status"
                aria-label={`${stat.title}: ${stat.value}. ${stat.sub}`}
              >
                <div>
                  <p className="text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-widest" aria-hidden="true">{stat.title}</p>
                  <p className="text-4xl font-black text-gray-900 dark:text-white tracking-tighter" aria-hidden="true">{stat.value}</p>
                  <p className="text-[10px] font-bold text-gray-400 dark:text-gray-500" aria-hidden="true">{stat.sub}</p>
                </div>
          ...
          {/* QUICK ACTIONS ROW */}
          <div 
            className="bg-white dark:bg-slate-800 rounded-[4rem] p-12 sm:p-20 border border-gray-100 dark:border-slate-700 text-center relative overflow-hidden"
            role="region"
            aria-labelledby="what-we-offer-title"
          >
            <Leaf className="absolute -top-10 -right-10 h-64 w-64 text-emerald-50 dark:text-emerald-900/20 rotate-12" aria-hidden="true" />
            <div className="mb-16 space-y-4 relative z-10">
              <h2 id="what-we-offer-title" className="text-4xl sm:text-5xl font-black text-gray-900 dark:text-white tracking-tighter">What We Offer</h2>
              <p className="text-gray-400 dark:text-gray-500 font-bold text-lg max-w-2xl mx-auto">Providing the best destination services in the world with sustainable management.</p>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 relative z-10">
              {[
                ...
                <button
                  ...
                  className="bg-white dark:bg-slate-700 p-10 rounded-[3rem] border border-gray-100 dark:border-slate-600 hover:border-emerald-200 dark:hover:border-emerald-700 hover:shadow-2xl transition-all group text-left active:scale-95 focus:outline-none focus:ring-4 focus:ring-emerald-500/10"
                  aria-label={`Go to ${action.title}: ${action.desc}`}
                >
                  <div className="w-16 h-16 rounded-2xl bg-emerald-50 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400 flex items-center justify-center mb-8 group-hover:scale-110 transition-transform">
                    <action.icon className="h-8 w-8" aria-hidden="true" />
                  </div>
                  <h3 className="font-black text-gray-900 dark:text-white text-xl mb-3 tracking-tight">{action.title}</h3>
                  <p className="text-gray-500 dark:text-gray-400 text-xs font-bold leading-relaxed">{action.desc}</p>
                  <div className="mt-6 flex items-center text-emerald-600 dark:text-emerald-400 font-black text-[9px] uppercase tracking-widest opacity-0 group-hover:opacity-100 transition-opacity">
